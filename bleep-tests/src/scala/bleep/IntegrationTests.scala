...
"""
  ...
  test("Require scalaVersion to be at least as high as the dependencies' scala-library") {
    ...
    runTest(
      ...
      val projectName = model.CrossProjectName(model.ProjectName("mytest"), None)
      assert(started.build.explodedProjects.contains(projectName))
      ...
      assert(thrown.getMessage.contains("bleep build evicted"))
      succeed
    }
  }
}
